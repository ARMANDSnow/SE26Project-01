...
- `db/migrations/runner.py` 提供连续版本迁移和事务回滚；当前 schema v10 支持 v2→v10 连续前迁，并对 Project/Item/Run/Artifact/dependency/Citation 及论文加工任务的列、FK、索引、唯一约束、JSON/hash 和 CHECK fail closed。

## Paper Processing Pipeline

- arXiv/USENIX/SIGOPS 的来源发现由用户手动触发；元数据与 `paper_processing_jobs` 在同一事务中写入，上传 PDF 亦在上传记录提交前幂等入队。
- `PaperProcessingExecutor` 是单机持久化队列的监督器。它只在短事务中领取任务、续租和结算；网络下载与 Docling 不占用 SQLite 写事务。
- Docling 在独立子进程中运行，父进程维持心跳并执行硬超时终止。任务使用 `lease_owner + lease_generation` 写栅栏，旧进程在租约过期或被接管后不能绑定 PDF、提交正文/Chunk 或宣告成功。
- 任务最多尝试固定次数并退避重试；一篇论文失败只更新本任务，执行器继续领取下一篇。HTTP PDF 路由只读取已缓存文件，不再因页面访问同步下载。
- 远程 PDF 先在临时流中校验大小、`Content-Length`、文件头和 EOF，再发布到内容寻址存储，失败下载不能删除或污染已存在的共享 blob。
...
- arXiv discovery 在单次外部请求中固定核心研究短语并对扩展词做 OR 组合，避免模型自由查询造成零召回或失控宽搜。远程 PDF 只有在 `Content-Length`（若提供）和 EOF 完整性校验通过后才可缓存/复用；截断文件在发布前明确失败，不能进入 Docling 或 source-hash 真相链。
...
- 模型输出不能直接成为数据库真相：研究任务整理、检索规划、候选筛选、阅读卡抽取、跨论文综合、对比矩阵和报告生成先通过 `extra='forbid'` 的版本化 schema，再经过身份、权限、source hash、Evidence/Citation 关系、事实文本白名单和脱敏检查；引用校验为确定性服务。报告生成器收到的是已验证 statement/key 精确白名单，只允许逐字选择，不能改写、合并、拆分或补造事实。
- 结构化模型调用把目标 Pydantic JSON Schema 写入系统提示；默认请求 provider JSON mode。不支持 `response_format` 的兼容 provider 必须显式配置 `LLM_JSON_RESPONSE_FORMAT=false`，返回值仍使用同一 strict schema fail closed。普通 Chat 默认解析 SSE 文本与兼容的分段 content；无文本时 fail closed，不能落成空白成功消息。仅支持非流式 Chat Completions 的网关可显式配置 `LLM_STREAMING=false`，应用仍用 SSE 返回一次 provider 调用的完整文本，不做流式失败后的隐藏 retry。
...
- `ResearchLandscapePlanner`、`TopicClusterGenerator`、`ResearchTimelineGenerator` 只接收服务端编制的 Paper/Claim/Citation 白名单；关系图构建与 `ResearchGraphValidator` 为确定性服务。Cluster 事实、Timeline 语义事件、Graph 语义边必须有当前有效 Citation；publication/precedes 仅表达已验证日期和时间排序。
...
## Offline Research Quality Evaluation

- `services/research_quality_evaluation.py` 是离线开发者工具，不挂载 Router、不读生产数据库，也不参与当前 Citation Validator。生产 schema/API/前端契约保持 v9。
- `rag-citation-entailment-v1` 使用公开 arXiv 身份和短 curator Evidence summary；严格 schema 校验未知字段、case ID、locator、Evidence SHA-256、标注状态与发布分布。draft 不进入评分。
- prediction 必须与全部 adjudicated case 一一对应。scorer 确定性生成 confusion matrix、macro-F1、supported precision、false-accept、语义 Citation coverage、确定性关系单列和 Artifact 分项结果。
- LLM judge 复用 `LLMClient` 的 OpenAI-compatible 配置与 JSON-mode 开关，每个案例固定 `max_attempts=1`。配置缺失、provider/schema/case ID 错误和敏感输出 fail closed；报告不记录 API Key、Authorization、provider body、base URL 或绝对路径。
- `prediction_file` 即使达到阈值也只标记“未验证”；只有本次 `llm_judge` 且 macro-F1、supported precision、coverage 同时达到阈值才标记“已达标”。该边界避免把结构 locator 校验或手工 prediction 包装成语义质量证明。
