...
主页 Chat 支持“自动判断 / 普通对话 / 深度研究”三种模式。普通 Chat 仍只发送系统提示和当前对话分支历史；深度研究会原子创建 17 步 `topic` Run，依次完成 ResearchBrief、检索/筛选、全文与 PaperBrief、综合计划、对比矩阵、跨论文 Claims、Citation Registry、严格引用验证和版本化研究报告，并把步骤、论文、Artifact、Evidence/Citation、工具摘要与预算持久化到数据库。远程 PDF 缓存和下载会校验长度与 EOF；报告事实只能逐字选用已验证的 statement/Citation 对。缺少真实模型配置时会明确失败为 configuration unavailable，不会生成伪调研数据。standalone `/api/research/runs` 仍保留确定性三步 Harness 作为可测试骨架。单篇论文 Chat 不进入 Research Route。

## 数据库 schema

当前 schema version 8 支持 v2→v3→v4→v5→v6→v7→v8 连续迁移。v5 新增 Research Run、Step、Event 和 Decision；v6 为 Chat 增加受控 `content_parts_json`；v7 新增版本化 `research_artifacts` 和 owner-scoped `research_run_papers`；v8 新增 durable model-call、opened Evidence 和 Citation ledger，并扩展综合/报告 Artifact。PaperBrief、Citation 与当前 PDF/Docling/Chunk source hash 和内容 hash 绑定。租约 generation 与每次副作用前后的 fencing 用于阻止过期 worker 覆盖新 attempt。更早的 `arxiv_id/file_path` v0 schema不提供自动迁移；不得在未备份或未确认归属时重建这类用户数据库。开发新功能可显式设置独立 `DATABASE_PATH`。只有确认没有需要保留的数据后，才执行破坏性重建：
