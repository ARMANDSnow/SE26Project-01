...
当前 schema version 5 支持 v2→v3→v4→v5 连续迁移。v5 新增 Research Run、Step、Event 和 Decision；租约 generation 用于阻止过期 worker 覆盖新 attempt。v2 的资料库、笔记、历史和订阅会迁移到用户归属模型，v3 的历史上传会标记为 `legacy public/approved`。v2 占位用户没有密码，迁移后会作为禁用的 legacy 账户保留数据。更早的 `arxiv_id/file_path` v0 schema 不提供自动迁移；不得在未备份或未确认归属时重建这类用户数据库。开发新功能可显式设置独立 `DATABASE_PATH`。只有确认没有需要保留的数据后，才执行破坏性重建：
...
- Research Harness：数据库驱动的可恢复 Run/Step/Event/Decision，支持安全暂停、继续、取消、重试和 SSE 续传；当前仅运行明确标记的确定性三步骨架，不生成伪调研结论
