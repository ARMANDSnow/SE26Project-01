...
主页 Chat 支持“自动判断 / 普通对话 / 深度研究”三种模式。普通 Chat 仍只发送系统提示和当前对话分支历史；深度研究当前只会原子创建可恢复的三步 Harness，不会伪装成论文调研结果。自动模式仅在规则无法确定时调用一次真实模型分类器；模型未配置或分类失败时不会写入消息或 Run，界面会要求显式选择模式。单篇论文 Chat 不使用 RAG：Docling 解析后的论文全文始终加入上下文。

## 数据库 schema

当前 schema version 6 支持 v2→v3→v4→v5→v6 连续迁移。v5 新增 Research Run、Step、Event 和 Decision；v6 为 Chat 增加受控 `content_parts_json`，旧文本会迁为 text part，`content` 继续作为模型上下文和旧契约的文本投影。租约 generation 用于阻止过期 worker 覆盖新 attempt。更早的 `arxiv_id/file_path` v0 schema 不提供自动迁移；不得在未备份或未确认归属时重建这类用户数据库。开发新功能可显式设置独立 `DATABASE_PATH`。只有确认没有需要保留的数据后，才执行破坏性重建：
...
- Research Harness：Chat 以 assistant-ui 原生 `research-run` data part 持久化 Run 卡片；共享 Workflow 支持安全暂停、继续、二次确认停止、重试、Decision 和 SSE 续传，当前仅运行确定性三步骨架
