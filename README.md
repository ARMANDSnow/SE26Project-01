...
当前 schema version 9 支持 v2→v3→v4→v5→v6→v7→v8→v9 连续迁移。v9 新增 owner-only 研究项目/成员、project Run mode、项目级追加版本 Artifact、规范化 dependency ledger 和项目 Citation reference。项目不扩大底层 Run/论文/报告权限，读取和写入均重验 ACL、Citation/Evidence 与 source hash；新上游版本或撤权会把下游项目分析投影为 stale/inaccessible。更早的 `arxiv_id/file_path` v0 schema 不提供自动迁移；不得在未备份或未确认归属时重建这类用户数据库。开发新功能可显式设置独立 `DATABASE_PATH`。只有确认没有需要保留的数据后，才执行破坏性重建：
