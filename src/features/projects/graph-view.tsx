...
export function GraphView({ projectId, artifact }: { projectId: string; artifact?: ResearchProjectArtifact<ResearchGraph> | null }) {
  ...
  const visibleNodes = useMemo(() => (content?.nodes ?? []).filter((node) => node.status !== "inaccessible"), [content?.nodes])
  const visibleIds = useMemo(() => new Set(visibleNodes.map((node) => node.node_id)), [visibleNodes])
  const visibleEdges = useMemo(() => (content?.edges ?? []).filter((edge) => edge.status !== "inaccessible" && visibleIds.has(edge.source_node_id) && visibleIds.has(edge.target_node_id)), [content?.edges, visibleIds])
    ...
    <ArtifactState artifact={artifact} empty="尚未生成研究关系图。">
      <div className="hidden min-w-0 gap-4 md:grid xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="min-w-0 overflow-hidden rounded-xl border bg-muted/20 p-2">
          <svg viewBox={`0 0 ${layout.width} ${layout.height}`} className="block h-auto max-h-[68vh] min-h-[420px] w-full" role="group" aria-label="可交互研究关系图">
            {visibleEdges.map((edge) => {
              ...
              const deterministic = ["contains", "generated_from", "cites", "precedes"].includes(edge.relation_type)
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <section><h3 className="mb-2 font-semibold">节点列表</h3><div className="grid gap-2">{(content?.nodes ?? []).map((node) => node.status === "inaccessible" ? <div key={node.node_id} className="rounded-xl border border-dashed p-3 text-sm text-muted-foreground">节点不可访问，内容已隐藏。</div> : <button key={node.node_id} type="button" aria-expanded={selectedNodeId === node.node_id} aria-controls="mobile-node-detail" className="min-h-14 min-w-0 rounded-xl border p-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={() => setSelectedNodeId(node.node_id)}><span className="block text-xs text-muted-foreground">{nodeTypeLabel[node.node_type]}</span><span className="block break-words text-sm font-medium [overflow-wrap:anywhere]">{node.label}</span></button>)}</div>{selectedNode ? <div id="mobile-node-detail" className="mt-3 rounded-xl border p-4 md:hidden" aria-live="polite"><NodeDetail projectId={projectId} node={selectedNode} /></div> : null}</section>
        <section><h3 className="mb-2 font-semibold">关系列表</h3><div className="grid gap-2">{(content?.edges ?? []).map((edge) => { const deterministic = ["contains", "generated_from", "cites", "precedes"].includes(edge.relation_type); return edge.status === "inaccessible" ? <div key={edge.edge_id} className="rounded-xl border border-dashed p-3 text-sm text-muted-foreground">关系不可访问，端点与类型已隐藏。</div> : <button key={edge.edge_id} type="button" className="min-h-14 min-w-0 rounded-xl border p-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={(event) => openEvidence(edge, event.currentTarget)}><span className="block break-words text-sm font-medium">{edgeTypeLabel[edge.relation_type]}</span><span className="mt-1 block text-xs text-muted-foreground">{deterministic ? "系统确定关系" : `有 ${edge.citation_keys.length} 条引用依据`}</span></button> })}</div></section>
      </div>
    </ArtifactState>
