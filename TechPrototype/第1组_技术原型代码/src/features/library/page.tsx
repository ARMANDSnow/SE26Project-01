...
function ProjectLibrary() {
  ...
  const projectsQuery = useResearchProjectsQuery(showArchived ? "archived" : "active")
  ...
  return <div className="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
    ...
    <div className="min-w-0"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><h2 className="text-lg font-semibold">{showArchived ? "已归档项目" : "进行中的项目"}</h2><p className="mt-1 text-sm text-muted-foreground">项目不会扩大 Run、论文、报告或引用的访问权限。</p></div><Button variant="outline" className="min-h-11" onClick={() => setShowArchived((value) => !value)}>{showArchived ? "查看进行中" : "查看已归档"}</Button></div>{projectsQuery.isLoading ? <LoadingState label="\u6b63\u5728\u8bfb\u53d6\u7814\u7a76\u9879\u76ee" skeleton /> : null}{projectsQuery.isError ? <div role="alert" className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-destructive/40 p-4 text-sm text-destructive"><span>\u7814\u7a76\u9879\u76ee\u8bfb\u53d6\u5931\u8d25\u3002\u8bf7\u786e\u8ba4\u4ecd\u5904\u4e8e\u767b\u5f55\u72b6\u6001\u540e\u91cd\u8bd5\u3002</span><Button variant="outline" onClick={() => void projectsQuery.refetch()}>\u91cd\u8bd5</Button></div> : null}<div className="grid gap-3 md:grid-cols-2">{projects.map((project) => <Link key={project.id} to={`/library/projects/${project.id}`} className="min-w-0 rounded-xl border p-4 hover:bg-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="flex min-w-0 items-start justify-between gap-2"><h3 className="break-words font-semibold [overflow-wrap:anywhere]">{project.title}</h3><Badge variant="outline">{project.status === "archived" ? "已归档" : "进行中"}</Badge></div>{project.description ? <p className="mt-2 line-clamp-2 break-words text-sm leading-6 text-muted-foreground">{project.description}</p> : null}<div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">{project.item_count != null ? <span>{project.item_count} 项资料</span> : null}{project.stale_item_count != null && project.stale_item_count > 0 ? <span>{project.stale_item_count} 项待更新</span> : null}<span>更新于 {formatLocalDateTime(project.updated_at)}</span></div></Link>)}</div>{!projects.length && !projectsQuery.isLoading && !projectsQuery.isError ? <AppEmptyState title={showArchived ? "没有已归档项目" : "还没有研究项目"} description={showArchived ? "归档后的项目会保留历史版本并显示在这里。" : "创建项目后，可以从 Run、论文和报告加入资料。"} /> : null}</div>
  </div>
}
...
function PaperLibrary() {
  ...
  const effectiveFolderId = selectedFolderId === root?.id ? undefined : selectedFolderId
  const itemsQuery = useLibraryItemsQuery(effectiveFolderId)
  ...
  const selectedFolder = selectedFolderId === undefined || selectedFolderId === root?.id
    ? root
