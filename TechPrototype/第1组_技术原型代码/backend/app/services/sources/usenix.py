...
from .common import MetadataPage


USENIX_BASE = "https://www.usenix.org/conference"


def fetch_usenix_papers(venue: str, year: int, max_results: int = 10) -> list[PaperCandidate]:
...
def _detail_to_paper(page: MetadataPage, url: str, venue: str, year: int) -> PaperCandidate | None:
    ...
    return PaperCandidate(
        ...
        source_id=f"{venue.lower()}:{year}:{external_id}",
        source_url=url,
        venue=f"{venue} {year}",
        title=title,
